...
const PrevArrow = ({ style, onClick, isVisible }: any) => {
  return (
    <motion.button
      ...
      type="button"
      className={`left-[-26px] top-[45%] z-30 absolute ${isVisible ? '' : 'hidden'}`}
      style={{ ...style,   }}
      onClick={onClick}
    >
     <GrFormPrevious
        className={`text-#fff hover:text-#72be43 hover:transition-colors hover:duration-700 hover:ease-in-out size-20 `}
      />
    </motion.button>
  );
};
...
const NextArrow = ({ style, onClick, isVisible }: any) => {
  return (
        <motion.button
          ...
          animate={{x: isVisible ? '-50%' : '-80%', opacity: isVisible ? 1 : 0}}
          type="button"
          className={`right-[-26px] top-[45%] absolute z-30 ${isVisible ? 'block' : 'none'}`}
          style={{ ...style}}
          onClick={onClick}
        >
          <GrFormNext
            className={`text-#fff hover:text-#72be43 hover:transition-colors hover:duration-300 hover:ease-in-out size-20 `}
          />
        </motion.button>
  );
};
