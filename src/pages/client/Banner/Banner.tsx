  ...
  return (
    <div className="bg-black">
      ...
      <div className=" flex justify-center mb-1.5">
          <img src="src/assets/images/BHDstarMember/image.png" alt="" className="mx-auto mb-1.5  h-28" />
        </div>
          ...
          {images.map((src, index) => (
            <div className="group relative" key={index}>
              <div className="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-md bg-gray-200 lg:aspect-none group-hover:opacity-75 lg:h-800">
                <img src={src} alt={`Image ${index + 1}`} className="h-full w-full object-cover object-center lg:h-full lg:w-full" />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="btn-submit-banner ">
        <Button className=" bg-black hover:bg-green-500 ">Đăng Kí Ngay</Button>
      </div>
    </div>
      </div>
    </div>
